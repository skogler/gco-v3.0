...
private:
	...
	struct node
	{
		...
		int			is_sink;	// flag showing whether the node is in the source or in the sink tree (if parent!=NULL)
		int			is_marked;	// set by mark_node()
		int			is_in_changed_list; // set by maxflow if 
